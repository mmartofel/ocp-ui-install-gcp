...
function initialize() {
  ...
  // Schema migration: track when a GPU MachineSet was created for the cluster
  try {
    db.exec('ALTER TABLE installs ADD COLUMN gpu_machineset_created_at INTEGER DEFAULT NULL');
  } catch (_) { /* column already exists */ }
...
function markGpuMachinesetCreated(installId) {
  db.prepare(`UPDATE installs SET gpu_machineset_created_at = ? WHERE install_id = ?`)
    .run(Date.now(), installId);
}
...
function purgeInstall(installId) {
  db.prepare('DELETE FROM install_logs WHERE install_id = ?').run(installId);
  db.prepare('DELETE FROM cluster_status WHERE install_id = ?').run(installId);
  db.prepare('DELETE FROM installs WHERE install_id = ?').run(installId);
}
